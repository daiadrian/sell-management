...
<div style="padding:10px 10px 10px 10px">
	<form id="curorderEditForm" class="itemForm" method="post">
		<input type="hidden" name="id"/>
	    <table cellpadding="5">
	        <tr>
	            <td>订单ID:</td>
	            <td><input class="easyui-textbox" id="curorderId" type="text" readonly="readonly" name="orderId" style="width: 280px;"/></td>
	        </tr>
			<c:if test="${sessionScope.get('paymentType') eq '堂食'}">
				<tr>
					<td>堂食单号:</td>
					<td><input class="easyui-numberbox" type="text" name="shoppingCode" readonly="readonly" /></td>
				</tr>
			</c:if>
			<c:if test="${sessionScope.get('paymentType') eq '外卖'}">
				<tr>
					<td>配送地址:</td>
					<td><input class="easyui-textbox" type="text" name="useraddress" readonly="readonly" /></td>
				</tr>
				<tr>
					<td>收餐人姓名:</td>
	...
	<div style="padding:5px">
		<a href="javascript:void(0)" class="easyui-linkbutton" onclick="confirmForm()">完成</a>
...
<script type="text/javascript">
    function reForm(){
        ...
        $("#curorderEditWindow").window('close');
        $("#curorderList").datagrid("reload");
    };
    function confirmForm(){
        var orderId = $("#curorderId").val();
        $.ajax({
            url:"${pageContext.request.contextPath}/order/current/manage/" + orderId,
            type:"GET",
            data:null,
            dataType:"json",
